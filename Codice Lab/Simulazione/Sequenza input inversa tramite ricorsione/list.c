...
int printNode(struct node *n){
    int count=0;
    if(n!=NULL){
        count= 1+ printNode(n->next);
        outputItem(n->value);
    }
    return count;
    if(n==NULL)
        return 0;
    return printNode(n->next)+1;
}

int printListRicorsiva(List lista){
    if(isEmpt(lista))
        return 0;
    return printNode(lista->head);
}
void printInverse(struct node* n,Stack stack){
    int count=0;
    if(n){
        count++;
        push(stack,n->value);
        printInverse(n->next,stack);
    }else{
        //char* str=toString(sizeStack(stack));
        //Item item=(Item)str;
        //sprintf();
        //push(stack,item);
        printStack(stack);
...
/*
int printInverse(struct node* n,Stack stack){
    int count=0;
    if(n){
        count= printInverse(n->next,stack)+1;
        outputItem(n->next);
        return  printInverse(n->next,stack);
    }
    return count;
...
*/
/*
void printListIterativaStack(List list){
    Stack stack=newStack();
     printInverse(list->head,stack);
}
*/