...
int altezzabst(BST bst){
    int left,right;
    if(isEmptyBST(bst) || isEmptyBST(getLeft(bst)) || isEmptyBST(getRight(bst)))
        return 0;
    else{
        left=altezzabst(bst->left);
        right=altezzabst(bst->right);
        if(left <right){
            return 1+right;
        }else{
            return 1+left;
...
int isBilanciato(BST bst){
    int altezzaSx=altezzabst(bst->left);
    int altezzaDx=altezzabst(bst->right);
    int differenza=altezzaSx-altezzaDx;
