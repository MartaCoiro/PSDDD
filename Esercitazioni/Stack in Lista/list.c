...
/**
 * @brief Realizzare delle funzioni che copi il
    contenuto di uno Stack in una Lista (mantenendo il contenuto dello Stack).
 * 
 * @param stack1 
 * @return List 
 */
List listToStack(Stack stack1){
    List list=newList();
    Stack stack2=newStack();
    while(!isEmptyStack(stack1)){
        Item item=top(stack1);
        addListTail(list,item);
        pop(stack1);
        push(stack2,item);
    }
    while(!isEmptyStack(stack2)){
        Item Item=top(stack2);
        push(stack1,Item);
        pop(stack2);
    }
    return list;
