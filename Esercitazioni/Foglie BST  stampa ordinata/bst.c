...
void printLeafRec(BST bst){
    if(!bst)
        return;
    if(getLeft(bst)==NULL && getRight(bst)==NULL)
        outputItem(bst->value);
    else{
        printLeafRec(bst->left);
        printLeafRec(bst->right);
    }
}
void printLeafOrder(BST bst){
    if(!isEmptyBST(bst))
        printLeafRec(bst);
}

void printLeafOrderIterative(BST bst){
    Queue queue=newQueue();
    enqueue(queue,bst);
    while(!isEsmptyQueue(queue)){
        BST temp=dequeue(queue);
        if(getLeft(temp)==NULL && getRight(temp)==NULL)
            outputItem(temp->value);
        if(getRight(temp)!=NULL)
            enqueue(queue,getRight(temp));
         if(getLeft(temp)!=NULL)
            enqueue(queue,getLeft(temp));
    }
}