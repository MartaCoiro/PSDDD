...
void printNodeRec(struct node* n){
    if(n){
        outputItem(n->value);
        printNodeRec(n->next);
    }
...
void printListRicorsiva(List list){
    printNodeRec(list->head);
}

Item searchNodeRec(struct node* n,Item item,int*pos){
    if(n==NULL){
        *pos=-1;
        return NULL;
    }
    if(compareItem(n->value,item)==0)
        ...
        (*pos)++;
        return searchNodeRec(n->next,item,pos);
    }
}

Item searchItemListRic(List list,Item item,int* pos){
    *pos=0;
    return searchNodeRec(list->head,item,pos);
}

int countNodeItemRec(struct node* n,Item item){
    if(n==NULL){
        return 0;
    }
    else{
        if(compareItem(n->value,item)==0){
            return countNodeItemRec(n->next,item)+1;
        }
        else return countNodeItemRec(n->next,item);
    }
}
/*
int countNodeItemRec(struct node* n,Item item){
...
*/
int countItemListRic(List list,Item item){
    return countNodeItemRec(list->head,item);
}
