...
/**
 * @brief Si effettura in ogni caso ua visita in preOrder, analizzando prima la radice
 *  poi il sottoalbero sinistro e poi il destro, da ricordare che per effettuare tale visita 
 * in modo iterativo è necessario inserire nello stack prima il destro e poi il sinistro 
 * 
 * @param b Btree da analizzare
 * @return int numero di elementi distinti
 */
int countDistinctItem(BTree b){
    int numNodi=nodiBtree(b);
    int count,pos;              // pos verrà inizializzato a 0 nella funzione searchList
    Stack stack= newStack();
    BTree bTemp;
    List list= newList();
    push(stack,b);
    for(count=0;count<numNodi;count++){
        bTemp=top(stack);
        if(searchList(list,bTemp->item,&pos)==NULL){
            addHead(list,bTemp->item);
        }
        pop(stack);
        if(!isEmptyTree(bTemp->right))
            push(stack,getRight(bTemp));
        if(!isEmptyTree(bTemp->left))
            push(stack,getLeft(bTemp));
    }
    return sizeList(list);
}

int countDistinctItemRicorsivo(BTree b,List list){
    int pos;
    if(!isEmptyTree(b)){
         if(searchList(list,b->item,&pos)==NULL){
            addHead(list,b->item);
        }
    countDistinctItemRicorsivo(b->left,list);
    countDistinctItemRicorsivo(b->right,list);
    }
    return sizeList(list);
}
