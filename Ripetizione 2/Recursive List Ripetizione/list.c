...
Item searchNodeRecursive(struct node* n,Item item,int* pos){
    if(!n){
        *pos=-1;
        return NULL;
    }else{
        if(compareItem(n->value,item)==0)
            ...
            (*pos)++;
            return searchNodeRecursive(n->next,item,pos);
        }
    }
}
...
Item searchItemRecursive(List list,Item item,int* pos){
    *pos=0;
    return searchNodeRecursive(list->head,item,pos);
}

int countNumItemNodeRecursive(struct node* n,Item item){
    if(!n)
        return 0;
    else{
        if(compareItem(n->value,item)==0)
            return countNumItemNodeRecursive(n->next,item)+1;
        return countNumItemNodeRecursive(n->next,item);
    }
}
int countNumItemRecursive(List list,Item item){
    return countNumItemNodeRecursive(list->head,item);
}