...
void printNodeRecursive(struct node* n){
    if(n){
        outputItem(n->value);
        printNodeRecursive(n->next);
    }
}
void printListRecursive(List list){
    if(!isEmpt(list))
        printNodeRecursive(list->head);
} 

Item searchNodeItemRecursive(struct node* n,Item item,int* pos){
    if(n==NULL){
        (*pos)=-1;
        return NULL;
    }
        ...
        if(compareItem(n->value,item)==0)
            ...
            (*pos)++;
            return searchNodeItemRecursive(n->next,item,pos);
        }
    }
}
Item searchItemRecursive(List list,Item item,int* pos){
    *(pos)=0;
    return searchNodeItemRecursive(list->head,item,pos);
}

int countItemNodeRecursive(struct node* n,Item item){
    if(n==NULL)
        return 0;
    else{
        if(compareItem(n->value,item)==0)
            return countItemNodeRecursive(n->next,item)+1;
        else return countItemNodeRecursive(n->next,item);
    }
}
int countNumItemRecursive(List list,Item item){
    return countItemNodeRecursive(list->head,item);
}

void destroyNodeRecursive(struct node* n){
    if(n){
        destroyNodeRecursive(n->next);
        free(n);
    }
}
void destroyListRecursive(List list){
    destroyNodeRecursive(list->head);
    list->head=NULL;
    list->size=0;
}