...
int searchItemRec(BTree b,Item item){
    if(isEmptyTree(b))
        return 0;
    if(cmpItem(b->item,item)==0 || searchItemRec(b->left,item) || searchItemRec(b->right,item))
        return 1;
}
int isEqualsRec(BTree b1,BTree b2){
    if(isEmptyTree(b1) && isEmptyTree(b2))
        return 1;
    if(isEmptyTree(b1) || isEmptyTree(b2))
        return 0;
    return cmpItem(b1->item,b2->item)==0 && isEqualsRec(getLeft(b1),getLeft(b2)) 
    && isEqualsRec(getRight(b1),getRight(b2));
}