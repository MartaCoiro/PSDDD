...
Item getMax(List list){
    struct node* max=list->head;
    struct node* p;
    for(p=list->head;p!=NULL;p=p->next){
        if(compareItem(p->value,max->value)>0)
            max=p;
    }
    return max->value;
}
Item getMin(List list){
    struct node* min=list->head;
    struct node* p;
    for(p=list->head;p!=NULL;p=p->next){
        if(compareItem(p->value,min->value)<0)
            min=p;
    }
    return min->value;
}
Item dueMinori(List);
char* concatItemeLessThenIterative(List list,Item item){
...
void concatRecursive(struct node* n,Item item, char* output){
    if(n){
        if(compareItem(n->value,item)<0){
            strcat(output,toString(n->value));
        }
        concatRecursive(n->next,item,output);
    }
}
char* concateItemLessThenRecursive(List list,Item  item){
    char* str=malloc(sizeof(char)*100);
    str[0]='\0';
    concatRecursive(list->head,item,str);
    return str;
}