...
void printLeafRecursive(BST bst){
    if(bst){
        printLeafRecursive(bst->left);
        if(getLeft(bst)==NULL && getRight(bst)==NULL)
            outputItem(bst->value);
        printLeafRecursive(bst->right);
    }
}
void printLeafIterativeInOrder(BST bst){
    if(!bst)
        return;
    else{
...
void printLeafIterativePreOrder(BST bst ){
    if(!bst)
        return;
    else{
        Stack stack=newStack();
        ...
        while(!isEmptyStack(stack)){
            ...
            if(getLeft(bstTemp)==NULL && getRight(bstTemp)==NULL)
                outputItem(bstTemp->value);
            pop(stack);
            if(getRight(bstTemp)!=NULL)
                push(stack,getRight(bstTemp));
            if(getLeft(bstTemp)!=NULL)
                push(stack,getLeft(bstTemp));
        }
    }
}