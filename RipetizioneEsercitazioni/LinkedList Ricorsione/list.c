...
void printNodeRecursive(struct node *p){
    if(p){
        outputItem(p->value);
        printNodeRecursive(p->next);
    }
}
...
int countItemNodeRecursive(struct node* p,Item item){
    if(p==NULL)
        return 0;
    else{
        if(compareItem(p->value,item)==0)
            return countItemNodeRecursive(p->next,item)+1;
        else return countItemNodeRecursive(p->next,item);
    }
}

int countItemRecursive(List list,Item item){
   return countItemNodeRecursive(list->head,item);
}

Item searchNodeItemRecursive(struct node* p,Item item,int* pos){
    if(p==NULL){
        *pos=-1;
        return NULL;
    }else{
        if(compareItem(p->value,item)==0){
            ...
            (*pos)++;
            return searchNodeItemRecursive(p->next,item,pos);
        }
    }
}
