...
void concatItem(struct node* n,Item item,char* out){
    if(n){
        if(compareItem(n->value,item)<0){
            strcat(out,toString(n->value));
        }
        concatItem(n->next,item,out);
    }
}

char* concatLessThenRecursive(List list,Item item){
    char* str=malloc(sizeof(char)*200);
    str[0]='\0';
    concatItem(list->head,item,str);
    return str;
