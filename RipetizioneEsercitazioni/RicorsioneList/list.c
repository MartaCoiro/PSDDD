...
Item searchNodeRic(struct node* n,Item item,int* pos){
	if(n==NULL){
		*pos=-1;
		return NULL;
	}
	if(compareItem(n->value,item)==0)
		...
		(*pos)++;
		return searchNodeRic(n->next,item,pos);
	}
}
Item searchListItemRecursive(List list,Item item,int* pos){
	*pos=0;
	return searchNodeRic(list->head,item,pos);
}


int countItemNodeRec(struct node* n,Item item){
	if(n){
		if(compareItem(n->value,item)==0)
			return countItemNodeRec(n->next,item)+1;
		return countItemNodeRec(n->next,item);
	}
	return 0;
}

int countItemListRecursive(List list,Item item){
	return countItemNodeRec(list->head,item);
}